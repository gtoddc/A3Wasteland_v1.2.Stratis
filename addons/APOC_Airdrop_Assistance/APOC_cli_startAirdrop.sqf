...
switch (_type) do {
	...
	case "base0":	{_selectionArray = APOC_AA_SupOptions};
...
if ((_timeRemainingReuse) > 0) then 
	{
		hint format["Air Drop OFFLINE! ETA to system back online %1 seconds", ceil _timeRemainingReuse];
...
_confirmMsg = format ["This airdrop will charge $%1 from your bank account on delivery<br/>",_price];
...
//	diag_log format ["AAA - Just Used Time: %1; CoolDown Set At: %2; Current Time: %3",APOC_AA_lastUsedTime, APOC_AA_coolDownTime, diag_tickTime];
	...
	sleep 1;
	hint format ["Airdrop Inbound %2 Heading: %1 ETA: 90s",ceil _heliDirection,_selectionName];
	player groupChat format ["Airdrop Inbound %2 Heading: %1 ETA: 90s",ceil _heliDirection,_selectionName];
	sleep 45;
	playSound3D ["a3\sounds_f\sfx\radio\ambient_radio14.wss",player,false,getPosASL player,1,1,25];
	sleep 1;
	hint format ["Airdrop Inbound %2 Heading: %1 ETA: 45s",ceil _heliDirection,_selectionName];
	player groupChat format ["Airdrop Inbound %2 Heading: %1 ETA: 45 seconds",ceil _heliDirection,_selectionName];
	};
